...
class ProductTemplateInherit(models.Model):
    ...
    # se agrega write_date para forzar el orden considerando múltiples name.
    # sea name un date o timestamp la última tasa será obtenida
    #TODO: debería estar en el módulo rates 
    def X_get_rates(self, company, date):
    ...
    @api.model
    # helper para obtener el factor de cambio desde el POS
    def get_conversion_rate(self,fromCurrencyName,toCurrencyName,datetime=fields.datetime.today()):
        company=self.env.company
        fromC=self.env['res.currency'].search([('name','=',fromCurrencyName)])
        toC=self.env['res.currency'].search([('name','=',toCurrencyName)])
        rate=self.env['res.currency']._get_conversion_rate(fromC,toC,company,datetime)
        return rate

#TODO: modificaciones de currency/rates esto no deben formar parte de este módulo
#TODO: debería estar en el módulo rates 
    ...
    def _process_payment_lines(self, pos_order, order, pos_session, draft):
        ...
        if not draft and not float_is_zero(pos_order['amount_return'], prec_acc):
            ...
            if not cash_payment_method:
                raise UserError("No cash statement found for this session. Unable to record returned cash.")
            return_payment_vals = {
                'name': _('return'),
