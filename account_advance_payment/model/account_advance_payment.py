...
class AccountAdvancePayment(models.Model):
    ...
    @api.depends('partner_id', 'journal_id', 'date_advance')
    ...
    def invoice_selected(self):
        if self.invoice_id:
            ...
            self.amount_currency_apply = self.invoice_currency
            if not self.date_apply:
                raise UserError("Establezca la fecha de aplicación")
            if self.currency_id.name == self.env.company.currency_id.name:
    ...
    company_currency = fields.Many2one('res.currency', string='Bs currency',
                                       default=lambda self: self.env['res.currency'].search([('symbol', '=', 'Bs.')]))
    ...
    def validate_amount_advance(self):
        if self.amount_advance <= 0:
            raise UserError("El monto de anticipo debe ser mayor que cero")

        return True

    def validate_amount_apply(self):
        if self.amount_apply <= 0:
            raise UserError("El monto a aplicar debe ser mayor que cero")
    ...
    def unlink(self):
        ...
        for move_id in self:
            if move_id.state not in ('draft','cancel'):
                raise UserError("You cannot delete an advance payment is not draft or cancelled")
    ...
    def copy(self, default=None):
        ...
        # Se crea una copia del anticipo cuando se cancela un anticipo que fue procesado completamente
            ...
            #raise UserError("El monto a aplicar (%s) no puede ser mayor al monto disponible")
    ...
    def validate_amount(self, vals):
        ...
        if amount_currency_apply == self.currency_id.id:
            if amount_apply > amount_available:
                raise UserError("El monto a aplicar (%s) no puede ser mayor al monto disponible (%s)" % (amount_apply, self.amount_available))

        else:
            if amount_apply > amount_available_conversion:
                raise UserError("El monto a aplicar (%s) no puede ser mayor al monto disponible (%s)" % (
                amount_apply, amount_available_conversion))
        if amount_currency_apply == invoice_currency:
            if amount_apply > amount_invoice:
                raise UserError("El monto a aplicar (%s) no puede ser mayor al monto de la factura (%s)" % (
            ...
            if amount_currency_apply == self.env.company.currency_id.id:
                if self.currency_id.id == self.env.company.currency_id.id:
                    if not currency_rate:
                        raise UserError(_('Registre una tasa de cambio para la fecha de aplicación'))
                ...
                if self.currency_id == self.env.company.currency_id.id:
                    if not currency_rate:
                        raise UserError("Registre una tasa de cambio para la fecha de aplicación")
                    conversion = amount_apply * currency_rate
                    if conversion > amount_invoice:
                        raise UserError("El monto a aplicar (%s) no puede ser mayor al monto de la factura (%s)" % (
                            conversion, amount_invoice))
    ...
    def get_move_register(self):
        ...
        if self.partner_id.es_cliente and not cuenta_acreedora and self.type_advance == False:
                raise UserError("El cliente no tiene configurado la cuenta contable de anticipo")
        elif self.partner_id.es_proveedor and not cuenta_deudora and self.type_advance:
                raise UserError("El socio no tiene configurado la cuenta contable de anticipo")
            ...
            if self.currency_id.name != self.env.company.currency_id.name:
                ...
                if not currency_rate:
                    raise UserError("Asegurese de tener la multimoneda configurada y registrar la tasa de la fecha del anticipo")
    ...
    def action_cancel(self):
        ...
        if self.state == 'available':
                ...
                for advance in self:
                    for move in advance.move_id:
                        ...
                        if len(move_reverse)==0:
                            raise UserError("No se reversaron los asientos asociados")
                        res = {'state': 'cancel'}
                        return super(AccountAdvancePayment, self).write(res)
            else:
                raise UserError("El anticipo ya tiene una aplicacion")
            ...
            if obj_move_line:
                raise UserError("El anticipo ya tiene una aplicacion en la factura %s" % self.invoice_id.name)
            else:
                for advance in self:
